# File: /opt/access_control/src/api/modules/sync_module.py
# Modulo gestione sincronizzazione server remoto
...
def load_config():
    ...
    if CONFIG_FILE.exists():
        try:
            ...
            add_log('ERROR', f'Errore caricamento configurazione: {e}')
    
    # Default configuration
    return {
        'url': 'https://partner.comunerende.it',
        'database': 'rende',
...
def save_config(config):
    ...
    try:
        ...
        add_log('ERROR', f'Errore salvataggio configurazione: {e}')
        return False
...
def api_sync_config():
    ...
    if request.method == 'GET':
        ...
        # The password field is already of type="password" in HTML
        ...
        try:
            ...
            if save_config(config):
                ...
                add_log('INFO', 'Configurazione server aggiornata')
                return jsonify({'success': True, 'message': 'Configurazione salvata'})
            else:
                return jsonify({'success': False, 'error': 'Errore salvataggio configurazione'})
...
def api_sync_schedule():
    ...
    try:
        ...
        config['sync_enabled'] = data.get('sync_enabled', False)
        config['sync_interval_hours'] = data.get('sync_interval_hours', 12)
        ...
        if save_config(config):
            ...
            return jsonify({'success': True, 'message': 'Schedulazione aggiornata'})
        else:
            return jsonify({'success': False, 'error': 'Errore salvataggio schedulazione'})
...
def initialize_sync_module():
    ...
    if config.get('sync_enabled', False):
        ...
        sync_thread = threading.Thread(target=sync_worker, daemon=True)
        sync_thread.start()
        add_log('INFO', 'Modulo sincronizzazione inizializzato')
